...
function getBoxSelectedPokemonAddr(pointer, partyAddr)
 ...
 if isBoxOpened() then
  ...
  if boxSelectedSlotIndex < 4 then  -- 未选择宝可梦槽位时索引值低于 4
   return boxAddr
  elseif boxSelectedSlotIndex >= 4 and boxSelectedSlotIndex <= 9 then
   return partyAddr + (0xC4 * (boxSelectedSlotIndex - 4))  -- 同行选择中的盒子槽位索引值从 4 开始
  end

  local currentBoxIndex = read8Bit(currentBoxIndexAddr)

  return boxAddr + (0xC4 * (boxSelectedSlotIndex - 10)) + (0x14 * currentBoxIndex) + (0xC4 * currentBoxIndex * 0x1E)  -- 盒子选择中的盒子槽位索引值从 10 开始
 end
...
function getPokemonInfoText(pointer, trainerTID, trainerSID)
 ...
 local boxPID, boxOTID, boxOTSID, boxSpeciesName, boxNatureName, boxHpIV, boxAtkIV, boxDefIV, boxSpAtkIV, boxSpDefIV, boxSpdIV, boxHpType,
       boxHpPower, boxCatchRateValue = getPokemonInfo(boxSelectedPokemonAddr)  -- 当前选中的宝可梦或第 1 个盒子的第 1 个槽位
