...
function setBackgroundBoxes()  -- 设置透明黑色框
 ...
 elseif mode[index] == "捕获" or mode[index] == "培育" or mode[index] == "C-Gear" or mode[index] == "宝可梦信息" then
...
function setPredictedDateTime()
 ...
 if dateTime["秒"] + 6 >= 60 then
  nextSeconds = (dateTime["秒"] + 6) - 60
  nextMinutes = dateTime["分钟"] + 1
 else
  nextSeconds = dateTime["秒"] + 6
...
function buildSeedFromDelay(delay, predictSeedFlag)
 ...
 local macAddr = 0xE4916  -- BizHawk MAC 地址
...
function handleMTAdvances(mtSeed, delay)
 if prevMTSeed ~= mtSeed and delay > 200 then  -- 检查 RAM 中 MT 种子值变化时机
  ...
  if convertToString(mtSeed) == convertToString(getMTArrayFistSeed(cgearSeedTest)) then  -- 检查 C-Gear MT 播种
   mtCounter = 0
...
function getInitialSeedInfoInput()
 ...
 gui.pixelText(1, 376, showCGearSeedInfoText and "7 - 隐藏 C-Gear 种子信息" or "8 - 显示 C-Gear 种子信息")
end

function showCGearSeedInfo(delay)
 if showCGearSeedInfoText then
  gui.drawBox(1, 192, 109, 255, 0x7F000000, 0x7F000000)
  gui.pixelText(1, 192, string.format("下一初始种子：%08X", buildSeedFromDelay(delay + 349, true)))
  gui.pixelText(1, 199, string.format("下一延迟：%d", delay + 349))
  gui.pixelText(1, 206, string.format("延迟：%d", delay))
  gui.pixelText(1, 220, string.format("C-Gear 种子：%08X", cgearSeed))
  gui.pixelText(1, 227, string.format("命中延迟：%d", hitDelay))
  gui.pixelText(1, 234, string.format("命中日期/小时：\n%s", hitDate))
 end
end
...
function showRngInfo()
 ...
 if showRngInfoText and mode[index] ~= "无" then
  ...
  gui.pixelText(0, 162, string.format("初始种子：%08X%08X", initialSeedHigh, initialSeedLow))
  gui.pixelText(1, 169, string.format("当前种子：%08X%08X", currentSeedHigh, currentSeedLow))
...
function getRngInfoInput()
 ...
 gui.pixelText(125, 183, showRngInfoText and "5 - 隐藏 RNG 信息" or "6 - 显示 RNG 信息")
end
...
function showMoves(moveIndexesList)
 gui.pixelText(1, 64, "招式："..moveNamesList[moveIndexesList[1] > 560 and 1 or moveIndexesList[1]])
 gui.pixelText(1, 71, "招式："..moveNamesList[moveIndexesList[2] > 560 and 1 or moveIndexesList[2]])
 gui.pixelText(1, 78, "招式："..moveNamesList[moveIndexesList[3] > 560 and 1 or moveIndexesList[3]])
 gui.pixelText(1, 85, "招式："..moveNamesList[moveIndexesList[4] > 560 and 1 or moveIndexesList[4]])
end
...
function showInfo(pidAddr)
 ...
 if mode[index] ~= "培育" or isEgg then
  gui.pixelText(1, 8, "种类："..speciesNamesList[(speciesDexIndex > 649 or speciesDexIndex < 1) and 1 or speciesDexIndex])
  ...
  gui.pixelText(1, 29, string.format("特性：%s (%s)", abilityNamesList[(abilityIndex > 164 or abilityIndex < 1) and 1 or abilityIndex],
  ...
  gui.pixelText(1, 50, "携带道具："..itemNamesList[(heldItemIndex > 639) and 1 or heldItemIndex])
